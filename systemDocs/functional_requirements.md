...
- Multi-role support using current stored roles: Super Admin, Coordinator, Reviewer/Supervisor, Student
...
- `systemDocs/Database_Stageing/database_schema.sql` — AMS domain schema (23 AMS tables, plus framework/package tables)
...
| FR-AU-005 | The system shall allow manual creation of individual user accounts via the UI. Super Admin manages Coordinator accounts; Coordinators manage non-admin staff and students. | Super Admin, Coordinator | Must | `users`, spatie permission tables | 1.0 |
| FR-AU-006 | The system shall allow bulk import of users via the shared Bulk Imports page using CSV, XLSX, or ODS files (university_id, name, email, role). Import uses validation preview and blocks import until errors are fixed. | Coordinator | Must | `users`, spatie permission tables | 1.0 |
| FR-AU-007 | The system shall allow assigning and revoking the stored roles `Super Admin`, `Coordinator`, `Reviewer/Supervisor`, and `Student` via spatie/laravel-permission. | Super Admin, Coordinator | Must | spatie permission tables | 1.0 |
...
| FR-AU-010 | The system shall allow staff with the `Reviewer/Supervisor` role to act as supervisor or reviewer based on project assignments, with the dashboard separating supervised projects from review assignments. | System | Must | `projects`, `project_reviewer`, spatie permission tables | 1.0 |
| FR-AU-011 | The system shall integrate brute-force login protection via Fail2Ban. | System | Should | None (infrastructure) | 1.0 |
| FR-AU-012 | The system shall support Coordinator self-registration through the Admin panel registration page, but new Coordinator accounts remain unapproved until a Super Admin approves them. | Coordinator, Super Admin | Must | `users.is_approved` | 1.0 |
...
| FR-MD-007 | The system shall redirect a Super Admin to the Master Data Setup Wizard while departments, specializations, courses, or grading scales are missing. The wizard persists progress on the user record. | Super Admin | Must | `departments`, `specializations`, `courses`, `grading_scales`, `users.master_data_setup_progress` | 1.0 |
...
| FR-TP-009 | The system shall allow importing rubric templates from formatted CSV, XLSX, or ODS files. The rubric-template importer supports multiple files in one upload; each file becomes one rubric template with deliverables, criteria, and score levels. Validation errors abort the import. | Coordinator | Must | `rubric_templates`, `deliverables`, `criteria`, `score_levels`, `rubric_folders` | 2.0 |
...
| FR-TP-011 | The system shall allow creating a phase template with a name, reviewer/external staff assignments, and calculated total_phase_marks. | Coordinator | Must | `phase_templates`, `phase_template_reviewer`, `phase_template_external` | 2.0 |
| FR-TP-012 | The system shall allow mapping rubric templates to a phase template via phase_rubric_rules, specifying evaluator_role responsibility label, fill_order, max_marks, and aggregation_method. | Coordinator | Must | `phase_rubric_rules`, `phase_templates`, `rubric_templates` | 2.0 |
...
| ID | Description | Actor(s) | Priority | Tables Touched | DFD Process |
|----|------------|----------|----------|---------------|-------------|
| FR-SA-001 | The system shall allow Super Admins to create a semester with name, academic_year, start_date, and end_date. | Super Admin | Must | `semesters` | 3.0 |
| FR-SA-002 | The system shall allow Super Admins to activate/deactivate a semester (is_active toggle). | Super Admin | Must | `semesters` | 3.0 |
| FR-SA-003 | The system shall allow Super Admins to close a semester (is_closed=true), making all data within it read-only. | Super Admin | Must | `semesters` | 3.0 |
| FR-SA-004 | The system shall scope Coordinator project access by `projects.coordinator_id`; Coordinators manage only projects assigned to them. | Super Admin, Coordinator | Must | `projects.coordinator_id` | 3.0 |
...
| FR-SA-006 | The system shall allow assigning one or more students to a project with no fixed project-size cap in the current implementation. | Coordinator | Must | `project_student` | 3.0 |
| FR-SA-007 | The system shall assign exactly one supervisor per project. The supervisor is a user with the `Reviewer/Supervisor` role and cannot also be a reviewer for the same project. | Coordinator | Must | `projects` | 3.0 |
...
| FR-SA-010 | The system shall allow bulk spreadsheet import for creating projects with student and supervisor assignments. Semester, course, phase template, specialization, and phase-template reviewers are selected in the UI context step after preview. | Coordinator | Must | `projects`, `project_student`, `project_reviewer` | 3.0 |
...
| FR-SA-013 | The system shall warn when a student is already assigned to another project in the same semester, then move the student to the new project only after explicit confirmation. Duplicate students inside the same import file remain validation errors. | System | Must | `project_student`, `projects` | 3.0 |
...
| ID | Description | Actor(s) | Priority | Tables Touched | DFD Process |
|----|------------|----------|----------|---------------|-------------|
| FR-AE-001 | The system shall dynamically render an evaluation form based on the rubric template's deliverables, criteria, and score levels. | Reviewer/Supervisor | Must | `evaluations`, `rubric_templates`, `deliverables`, `criteria`, `score_levels` | 4.0 |
| FR-AE-002 | For criteria where is_individual=false (group), the system shall accept a single score that applies to the entire project (student_id=NULL in evaluation_scores). | Reviewer/Supervisor | Must | `evaluation_scores` | 4.0 |
| FR-AE-003 | For criteria where is_individual=true, the system shall require a separate score for each student in the project (one evaluation_score row per student). | Reviewer/Supervisor | Must | `evaluation_scores` | 4.0 |
| FR-AE-004 | The system shall allow saving an evaluation as draft (status=draft), preserving all entered scores as editable. | Reviewer/Supervisor | Must | `evaluations` | 4.0 |
| FR-AE-005 | The system shall allow submitting an evaluation (status=submitted), which locks all scores from further editing. | Reviewer/Supervisor | Must | `evaluations` | 4.0 |
| FR-AE-006 | The system shall allow evaluators to select a predefined score level from a dropdown or enter a manual score within the criterion's max_score range. | Reviewer/Supervisor | Must | `evaluation_scores`, `score_levels` | 4.0 |
| FR-AE-007 | The system shall allow per-criterion text feedback that is visible to students. | Reviewer/Supervisor | Must | `evaluation_scores` | 4.0 |
| FR-AE-008 | The system shall allow general feedback per evaluation. | Reviewer/Supervisor | Should | `evaluations` | 4.0 |
...
| FR-RE-002 | The system shall provide a staff dashboard showing all supervised projects and their evaluation progress for users with the `Reviewer/Supervisor` role. | Reviewer/Supervisor | Must | `projects`, `evaluations` | 6.0 |
| FR-RE-003 | The system shall provide a `Reviewer/Supervisor` dashboard with clearly separated sections: "Projects I am Supervising" and "Projects I am Reviewing". | Reviewer/Supervisor | Must | `projects`, `project_reviewer`, `evaluations` | 6.0 |
...
|--------|-----------|-------------|-----------------|-------------------|
| Authentication & User Management | FR-AU | 1.0 Manage Master Data | Domain 1: Users & Master Data | 12 |
| Master Data Management | FR-MD | 1.0 Manage Master Data | Domain 1: Users & Master Data | 7 |
...
|----------|-------|-------------|
| **Must** | 66 | Required for MVP. System cannot function without these. |

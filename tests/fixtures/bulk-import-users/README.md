...
- `10_invalid_student_id_patterns.csv`: student IDs that violate the requested business format.
- `11_invalid_staff_id_and_email_patterns.csv`: staff IDs and staff email conventions that should fail business-rule validation.
- `12_column_mapping_nonstandard_headers.csv`: valid rows with non-standard headers for the column-mapping step.
- `13_empty_headers_only.csv`: headers only, no data rows.
- `14_valid_staff_role_aliases.csv`: valid staff aliases accepted by import and normalised to `Reviewer/Supervisor`.
